...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        # Deteksi hanya laptop dan handphone
        from utils.detector import deteksi_perangkat
        hasil_path, jumlah_perangkat = deteksi_perangkat(filepath)

        if not hasil_path:
            return "Gagal memproses file. Format tidak didukung atau error saat deteksi.", 400

        filename_only = os.path.basename(hasil_path)
        return render_template('result.html', hasil=filename_only, jumlah=jumlah_perangkat)
